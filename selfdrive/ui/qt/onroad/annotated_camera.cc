...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  try {
    ...
    logError(__func__, __FILE__, __LINE__, e.what());
  } catch (...) {
    logError(__func__, __FILE__, __LINE__);
  }
}
...
void AnnotatedCameraWidget::initializeFrogPilotWidgets() {
  try {
    ...
    cemOverriddenIcon = loadPixmap("../assets/img_couch.svg", {img_size / 2, img_size / 2});
    ...
    experimentalEnabledIcon = loadPixmap("../assets/img_experimental.svg", {img_size / 2, img_size / 2});
...
void AnnotatedCameraWidget::drawCEMStatus(QPainter &p) {
  try {
    ...
    QPixmap iconToDraw;
    if (conditionalStatus == 1 || conditionalStatus == 3 || conditionalStatus == 5) {
      iconToDraw = cemOverriddenIcon.scaled(iconSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
    } else if (conditionalStatus == 2 || conditionalStatus == 4 || conditionalStatus == 6) {
      iconToDraw = experimentalEnabledIcon.scaled(iconSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
