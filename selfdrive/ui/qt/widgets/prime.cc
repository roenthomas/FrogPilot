...
void PairingQRWidget::refresh() {
  ...
  QString qrString = (uiState()->scene.use_frogpilot_server ? "https://portal.springerelectronics.com/?pair=" : "https://connect.comma.ai/?pair=") + pairToken;
...
PairingPopup::PairingPopup(QWidget *parent) : DialogBase(parent) {
  ...
  hlayout->addLayout(vlayout, 1);
  {
    ...
    QLabel *title = new QLabel(uiState()->scene.use_frogpilot_server ? tr("Pair your device to your springer account") : tr("Pair your device to your comma account"), this);
    title->setStyleSheet("font-size: 75px; color: black;");
    title->setWordWrap(true);
    vlayout->addWidget(title);

    QString serverUrl = uiState()->scene.use_frogpilot_server ? "portal.springerelectronics.com" : "connect.comma.ai";
